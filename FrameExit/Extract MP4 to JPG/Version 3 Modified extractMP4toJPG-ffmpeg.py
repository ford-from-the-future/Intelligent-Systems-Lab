...
#annotFile = "/home/six-path-sage/Downloads/DownloadedVideo2023/activity_net.v1-3.min.json"

#baseDir = UNCOMMENT THIS OUT AND HAVE IT POINT TO A DIRECTORY CONTAINING A SUBDIRECTORY CALLED VIDEOS
...
def saveVidInfo(vids):
    ...
    for idx, vid in enumerate(vids):
        ...
        vinfo = dict()
        vinfo['duration'] = float(
            ffdata['duration'])  # frame rate info provided by ffprobe does not always match with ffmpeg framte_rate
        vinfo['extract-num-frames'] = len(imglist)
        vinfo['read-num-frames'] = int(
            ffdata['nb_frames'])  # nb_frame provided by ffprobe does not always match with ffmpeg number of frames
        vinfo['frame-rate-info'] = ffdata[
            'r_frame_rate']  # frame rate info provided by ffprobe does not always match with ffmpeg frame_rate
        r_rate = ffdata['r_frame_rate'].split('/')
        r_rate = float(r_rate[0]) / float(r_rate[1])
        vinfo['frame-rate'] = r_rate
        vinfo['avg-frame-rate'] = ffdata[
            'avg_frame_rate']  # frame rate info provided by ffprobe does not always match with ffmpeg frame_rate
